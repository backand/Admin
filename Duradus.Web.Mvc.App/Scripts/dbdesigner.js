...
var LOCALE = {};
var SQL = {};
var loadDBXmlFuncName = "Durados/GetERDXml";
/* -------------------- base visual element -------------------- */
...
SQL.Row.prototype.redraw = function() {
	...
	if (true/*.owner.owner.getOption("showtype")*/) {
		var elm = this.getDataType();
...
SQL.Map.prototype.sync = function() { /* when window changes, adjust map */
	...
	this.w = Math.max(Math.round(w), 0);
	this.h = Math.max(Math.round(h), 0);
	this.l = Math.max(Math.round(x), 0);
	this.t = Math.max(Math.round(y), 0);
...
SQL.IO.prototype.init = function(owner) {
	...
	var ids = ["saveERD","saveload", "clientsave", "clientload", "clientsql", 
	...
	OZ.Event.add(this.dom.saveERD, "click", this.bind(this.saveERD));
...
SQL.IO.prototype.saveERD = function () {
    var xml = this.owner.toXML();
    var h = { "Content-type": "text/xml" };
    var h = { };
    this.owner.window.showThrobber();
    this.owner.setTitle(name);
    xml = encodeURIComponent(xml);
    OZ.Request(saveStateUrl, null, { xml: true, method: "post", data: "p=" + xml, headers: h });
}
...
SQL.IO.prototype.clientsql = function() {
    ...
    var path = bp + "Config/Sql/mssql/"+window.DATATYPES.getAttribute("db")+"/output.xsl";
	this.owner.window.showThrobber();
	OZ.Request(path, this.bind(this.finish), {xml:true});
}
...
SQL.IO.prototype.serverload = function (e, keyword) {
    ...
    var url = bp + "/" + keyword + "?rnd=" + Math.random();
    this.owner.window.showThrobber();
    this.name = name;
...
SQL.Designer.prototype.requestDB = function () { /* get datatypes file */
    ...
    var bp = this.getOption("staticpath");
    bp = "";
    var url = bp + "../../Config/sql/" + db + "/datatypes.xml";
...
SQL.Designer.prototype.init2 = function () { /* secondary init, after locale & datatypes were retrieved */
    ...
    var r = url.match(/appName=([^&]+)/);
    if (r) {
        var appName = r[1];
        //alert("Config/" + appName + ".ERD.xml?rnd=" + Math.random());
        var action = "Config/" + appName + ".ERD.xml?rnd=" + Math.random();
        action = "Admin/LoadERDXML";
        this.io.serverload(false, action);
...
SQL.Designer.prototype.removeSelection = function () {
    ...
    if (!sel) { return; }
    try { if (sel.empty) { sel.empty(); } } catch (err) { }
